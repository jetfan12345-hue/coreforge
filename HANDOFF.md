...
- Persist profile in Zustand (`src/store/fitness.ts`). `demoModel` and `coachTrashTalk` merge on rehydrate. First-open is **Get to it** (train now, floor defaults) or optional **Enter some details** — never a 3-step quiz.
